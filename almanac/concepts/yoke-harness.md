...
Before many operations, the harness resolves required features. `run`, `stream`, `start`, `workflow`, `models`, and session-management methods all call capability planning helpers before delegating to an adapter [@models]. This is where the harness connects to [Provider Surfaces](provider-surfaces): the selected surface must support the features implied by the agent and options. For the exact selection and validation flow, read [Capability Planning](../architecture/capability-planning). See [Workflows](workflows) for the split between portable Yoke orchestration and provider-native workflow artifacts.
